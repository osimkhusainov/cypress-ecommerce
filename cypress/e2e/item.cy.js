...
describe('Item page with a valid item', () => {
  ...
  it('Should render mocked img', () => {
    cy.findByAltText(name).should('have.attr', 'src').and('include', image_url);
  });

  it('Should render mocked description', () => {
  ...
  it('All starts should be gray and "No ratings available" text', () => {
    cy.get("[data-forhalf='★']")
      .should('have.length', 5)
      .each((star) => {
        cy.wrap(star).invoke('attr', 'style').should('include', 'color: gray;');
      });
  });
